...
class HomeActivityDuplicate : AppCompatActivity() {
...
//        checkSubmissionStatus("TASK3")
        ...
        btn_guide.setOnClickListener {
            ...
            val intent = Intent(this@HomeActivityDuplicate, FinalDocumentsActivity::class.java)
...
//            startAnimationCustom()
    ...
    //REMOVE: Launch this only when previous task is submitted
    private fun taskSelector(pass: String) {
        val intent = Intent(this@HomeActivityDuplicate, SelectTaskActivity::class.java)
        intent.putExtra("task",pass)
        intent.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TASK or Intent.FLAG_ACTIVITY_NEW_TASK)
        startActivity(intent)
    }

    //REMOVE: Launch this only when final task is submitted
    private fun completeInternShip() {
        val userUID = FirebaseAuth.getInstance().currentUser?.uid.toString()
        Toast.makeText(this@HomeActivityDuplicate, "Data Parceled and Uploaded", Toast.LENGTH_LONG).show()
    ...
    //FIXME:Donot delete any data when internship is completed
        ...
        historyNode.push().updateChildren(taskMap).addOnCompleteListener { task->
            ...
            FirebaseDatabase.getInstance().reference.child("Users").child(userUID)
                .child("currentActivated").removeValue().addOnCompleteListener { task->
                    ...
                    val intent = Intent(this@HomeActivityDuplicate, EndedActivity::class.java)
                    intent.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TASK or Intent.FLAG_ACTIVITY_NEW_TASK)
    ...
    private fun nextCode() {
        ...
        fab_help.setOnClickListener {
            Toast.makeText(this@HomeActivityDuplicate,"You May Join this Group",Toast.LENGTH_SHORT).show()
    ...
    //REMOVE: DO not auto run this mf
   ...
   //check what info to save
    ...
    taskNode.updateChildren(taskMap).addOnCompleteListener { task->
        if(task.isSuccessful){
            Toast.makeText(this@HomeActivityDuplicate,"$taskNo completed succesfully!",Toast.LENGTH_SHORT).show()
            if(taskNo == "TASK1"){
                val newTaskNo = "TASK2"
    ...
    private fun initActivity() {
        ...
        Glide.with(this@HomeActivityDuplicate).load(user!!.photoUrl).into(iv_profile)
        tv_user_welcome.text = "Hello, $userName"
    }


    //checks todays day and further calls an function to set the current day in heading of activity
...
//                        setOngoingProblem(pass)
                        ...
                        //REMOVE: Dont show this message at end
                        //day exceeds 30
